...
const SmartHouse = () => {
  ...
  const handleAdd = () => {
    if (addValue.trim() === "") {
      // Не добавлять пустые устройства
      return;
    }
    setDevice([
      ...device,
      { id: Date.now(), name: addValue, isPower: false, isEdit: false },
    ]);
    setAddValue("");
  };
  ...
  const handleEdit = (isDevice) => {
    if (editValue.trim() === "") {
      // Не редактировать устройство с пустым названием
      return;
    }
    setDevice(() =>
      device.map((el) => {
  ...
  return (
    <div className={style.cont}>
      ...
      <div>
        ...
        />
        <button onClick={handleAdd} className={style.btn}>
          Добавить устройство
        </button>
      </div>
      <div>
